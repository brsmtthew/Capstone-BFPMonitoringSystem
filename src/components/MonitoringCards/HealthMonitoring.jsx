...
const HealthMonitoring = ({ monitoringHealthData = [] }) => {
  return (
    <div className="bg-white w-full rounded-lg shadow-md xl:h-96 2xl:h-96">
      ...
      <div className="font-montserrat grid gap-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3  mt-4 p-4">
        {monitoringHealthData.map((data, index) => (
          <div
            key={index}
            // For the third card (index 2), span two columns on md and lg screens
            className={index === 2 ? "sm:col-span-2 md:col-span-2 lg:col-span-2 xl:col-span-1" : ""}
          >
            <HealthCard 
              icon={data.icon} 
              title={data.title}
